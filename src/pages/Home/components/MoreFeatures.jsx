            ...
            {features.map((item, index) => (
              <div
                ...
                <div className="relative flex flex-col items-center h-full justify-between z-10">
                  <h3 className=" text-center relative z-10 ">{item.title}</h3>
                  <img
                    className={` mx-auto byop__card-img${index}   max-w-full relative z-10`}
                    src={item.src}
                    alt={item.title}
                  />
                  <p className="byop__card-description text-center relative z-10">
                    {item.description}
                  </p>
                </div>

                <div className="absolute -bottom-52 -right-36 bottomGradient"></div>
              </div>
            ))}
          </div>
        </div>
